		...
		if(isset($proposedArticleDetails) && count($proposedArticleDetails) > 0){
			...
			if(isset($floorIds) && count($floorIds) > 0){
				...
				foreach($floorDetails as $floor){
					...
					//area
					$buildingArea = isset($buildingDetails['std_floor_space']) && $buildingDetails['std_floor_space'] != "" ? $buildingDetails['std_floor_space'].' 坪' : "";
...
// 					echo '<pre>'; print_r($floor);die;
	...
	public function createExcelSheet($objPHPExcel, $columnNames, $columnsData, $sheetIndex = 0) {
		...
		// for($i=1;$i< SHEET_SEPARATE_COLUMNS; $i++){
		// 	//print_r($i);
		// 	for($j=1;$j<$row;$j++){
		// 			$rows = $j;
			...
			foreach ($valData as $val)
			{
				...
				if(strpos($val,'buildingPictures') !== false){
					...
					if($row==14){
						...
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($from_cell,'14', $changeval[0]);
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($to_cell,'14', $changeval[1]);
