...
class CustomerSource extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('source_name, added_on, added_by, modified_on, modified_by', 'required'),
			array('is_active, added_by, modified_by', 'numerical', 'integerOnly'=>true),
			array('source_name', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('customer_source_id, source_name, is_active, added_on, added_by, modified_on, modified_by', 'safe', 'on'=>'search'),
		);
	}
