...
<ul class="breadcrumb">
    ...
    <li class="bActive"><i class="fa fa-angle-double-right" aria-hidden="true"></i> <?php echo Yii::app()->controller->__trans('Construction Types List'); ?></li>
</ul>
<hr/>
  <?php
...
$('.search-form form').submit(function(){
	$('#construction-type-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
");
?>
<h3><?php echo Yii::app()->controller->__trans('Construction Types List'); ?><i class="fa fa-plus btnAddNew" title="Add New Type"></i></h3>
...
<div class="modal-box hide">
  <div class="content">
    <div class="box-header">
      ...
      <h2 class="popup-label"><?php echo Yii::app()->controller->__trans('Add New Construction Type'); ?></h2>
    </div>
	...
	'columns'=>array(
		...
		array(
			...
			'value'=>function($data,$row){
				if($data->is_active==1){
					return '<a href="'.Yii::app()->createUrl('constructionType/changestatus',array('id'=>$data->construction_type_id,'is_active'=>'0')).'" class="btn btn-danger btn-sm ajax-status"><i class="fa fa-check"></i></a>';
				}
				if($data->is_active==0){
					return '<a href="'.Yii::app()->createUrl('constructionType/changestatus',array('id'=>$data->construction_type_id,'is_active'=>'1')).'" class="btn btn-success btn-sm ajax-status"><i class="fa fa-times"></i></a>';
				}
			},
			'type'=>'raw'
		),
		...
		array(
			...
			'buttons' => array(
				'update'=>array(
						...
						'icon'=>true,
						'url'=>'Yii::app()->createUrl("constructionType/getTypeData", array("id"=>$data->construction_type_id))',
						'imageUrl'=>false,
				...
				'delete'=>array(
						...
						'icon'=>true,
						'url'=>'Yii::app()->createUrl("constructionType/delete", array("id"=>$data->construction_type_id))',
						'imageUrl'=>false,
				),
			),
		),
...
$(document).ready(function(e) {
	...
	$(document).on('click','.btnModalClose',function(e){
		...
		$('.form-error').html('');
		$('.popup-label').html('Add New Construction Type');
	...
	$(document).on('click','.ajax-link',function(e){
		...
		call({url:url,params:{},type:'GET'},function(resp){
			...
			$('.popup-label').html('Update Construction Type');
	...
	$(document).on('click','.ajax-status',function(e){
		...
		call({url:url,params:{},type:'GET'},function(resp){
			//alert('Successfully Data Added');
			if(resp.success == 1){
				$.fn.yiiGridView.update('construction-type-grid');
			}else{
...
function addUpdateType(){
	var typeName = $('#typeName').val();
		if(typeName != ''){
		var id = $('#id').val();
		var url = '<?php echo Yii::app()->createUrl('constructionType/createOrUpdate'); ?>';
